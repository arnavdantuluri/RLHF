...
from utils import get_prompts, get_prompts_oasst_only
...
from utils import MiniDataset
from utils import print_rank_0, to_device, save_hf_format, set_random_seed, get_all_reduce_mean, moving_average, save_zero_three_model
...
def parse_args():
    ...
    parser.add_argument('--shuffle',
                        type=bool,
                        default=True,
                        help="Whether or not to shuffle DataLoader")
...
def create_datasets(args, prompts: List[str], tokenizer: PreTrainedTokenizer, train_phase=3):    
    model_inputs = tokenizer(
            prompts,
            truncation=True,
            padding=True,
            max_length=args.max_prompt_seq_len,
    ...
    collate_fn = DataCollatorWithPadding(tokenizer) if tokenizer else torch.vstack
    prompt_train_dataloader = DataLoader(prompts, batch_size=args.per_device_train_batch_size, collate_fn=collate_fn, shuffle=args.shuffle)
    ...
    return prompt_train_dataloader, [None] * len(prompt_train_dataloader), num_total_iters
...
def main():
    ...
    prompt_train_dataloader, unsupervised_train_dataloader, num_total_iters = create_datasets(
        args=args, prompts=get_prompts_oasst_only("2023-03-13_oasst_ready_labels.jsonl.gz", tokenizer), tokenizer=tokenizer, train_phase=3)
