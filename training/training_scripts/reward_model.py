...
class RewardModel(nn.Module):
    ...
    def __init__(self, base_model, tokenizer, model_ckpt_path, num_padding_at_beginning=0):
        super().__init__()
        AutoConfig.register("gpt_neox_reward_model", GPTNeoXRewardModelConfig)
        AutoModel.register(GPTNeoXRewardModelConfig, GPTNeoXRewardModel)
        AutoModelForCausalLM.register(GPTNeoXRewardModelConfig, GPTNeoXRewardModel)
        model = AutoModelForCausalLM.from_pretrained(model_ckpt_path)
        config = AutoConfig.from_pretrained(model_ckpt_path)
        ...
        self.rwtranrsformer = model
        self.v_head = self.rwtranrsformer.out_proj#nn.Linear(config.hidden_size if hasattr(config, "hidden_size") else config.n_embd,
                                    #1,
                                    #bias=False)
