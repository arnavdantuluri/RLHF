'''
NOTE: This is not nearly complete; Still waiting on the OA dataset to be modified in a way it can be used
'''
import os
...
def main(hparams={}):
    ...
    rm_model.half()
    rm_model.eval()
    rm_device = torch.device("cuda:{}".format(1))  # set reward model device
    ...
    def reward_fn(samples):
        ...
        for i in range(0, len(samples), batch_size):
            ...
            input_ids = encodings_dict["input_ids"].to(rm_device)
            attn_masks = encodings_dict["attention_mask"].to(rm_device)
            input_ids = input_ids.repeat(2, 1)
            attn_masks = attn_masks.repeat(2, 1)
            with torch.no_grad():
                sub_scores = rm_model(input_ids=input_ids, attention_mask=attn_masks)
